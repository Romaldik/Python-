...
def get_team_id(name):
    return db.get_data('id', 'team', name, 'name')

def get_team_id_for_post(name):
...
class Team:
    ...
    def add_player(nickname, team):
        ...
        db.update_data('player', 'team_id', get_team_id_for_post(team), 'id', player_id[0])

    def add_coach(nickname, team):
        coach_id = db.get_data('id', 'coach', nickname, 'nickname')
        db.update_data('coach', 'team_id', get_team_id_for_post(team), 'id', coach_id[0])

    def add_staff(name, team):
        staff_id = db.get_data('id', 'staff', name, 'name')
        db.update_data('staff', 'team_id', get_team_id_for_post(team), 'id', staff_id[0])

    def add_training_program(program_name, team):
        program_id = db.get_data('id', 'trainingprogram', program_name, 'name')
        db.update_data('team', 'training_program_id', get_team_id_for_post(team), 'id', program_id[0])
    ...
    def delete_sponsor(team, sponsor):
        ...
        db.delete_data('team_sponsor', ['sponsor_id', get_team_id_for_post(team), 'team_id', sponsor_id])
